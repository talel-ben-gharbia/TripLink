...
const N8N_CHATBOT_URL = "http://localhost:5678/webhook/chat"

const initialMessages = [
  { id: "m-0", text: "Hi! How can I help you today?", sender: "bot", timestamp: new Date() },
]
...
function Chatbot() {
  ...
  const handleSubmit = async (e) => {
    ...
    const thinkingMessage = { text: "Thinking...", sender: "bot", id: Date.now() + 1, isLoading: true }
    ...
    try {
      ...
      setMessages((prevMessages) => {
        const newMessages = prevMessages.filter((msg) => msg.id !== thinkingMessage.id)
        return [...newMessages, { id: Date.now() + 2, text: botReply, sender: "bot", timestamp: new Date() }]
      })
    } catch (error) {
      ...
      setMessages((prevMessages) => {
        ...
        return [
          ...newMessages,
          { id: Date.now() + 3, text: "Sorry, connection error.", sender: "bot", error: true, timestamp: new Date() },
        ]
      })
    } finally {
      setIsLoading(false)
      ...
      >
        ...
        <div className="px-4 py-4 bg-gradient-to-r from-indigo-500 via-indigo-600 to-indigo-700 text-white flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-white/20 backdrop-blur rounded-full flex items-center justify-center text-sm font-bold">✈</div>
            <div>
              <div className="text-sm font-semibold leading-tight">TripLink</div>
              <div className="text-xs opacity-90">Your travel companion</div>
            </div>
          </div>
          <button onClick={() => setOpen(false)} className="text-white/80 hover:text-white/100 transition-colors">
        ...
        <div className="p-4 flex-1 overflow-y-auto bg-gradient-to-b from-slate-50 to-white space-y-3">
          {messages.map((msg) => (
            ...
            >
              ...
              >
                <div className={`text-sm ${msg.sender === "user" ? "text-white" : "text-slate-500"} text-xs mb-0.5 opacity-70`}>
                  {msg.sender === "user" ? "You" : "TripLink"}
                </div>
                <div className={`text-sm leading-relaxed ${msg.isLoading ? "flex items-center gap-1" : ""}`}>
                  {msg.isLoading ? (
                    <>
                      <span className="inline-block w-2 h-2 bg-slate-400 rounded-full animate-bounce"></span>
                      <span className="inline-block w-2 h-2 bg-slate-400 rounded-full animate-bounce" style={{ animationDelay: "0.2s" }}></span>
                      <span className="inline-block w-2 h-2 bg-slate-400 rounded-full animate-bounce" style={{ animationDelay: "0.4s" }}></span>
                    </>
                  ) : (
                    msg.text
                  )}
